...
async function main(){
  ...
  let tickets = [];
  let [min, max] = config.FlightCapacityRandom;
  for(let i=0; i < config.ToTalFlight; i++){
    let id = 100000 + i;
    let capacity = _.random(min, max);
    let initData =  _.clone(Flight.getDefault());
    initData.capacity = capacity;
    // save flight info
    Flight.calPrice(initData);
    let flightId = 'flight_' + id;
    Flight.saveData(flightId, initData);
    for(let j = 1; j <= capacity; j++){
      let ticketId = `ticket_${id}_${j}`;
