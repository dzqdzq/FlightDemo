...
function subExpired (err:Error|null, res:'OK') {
  ...
  sub.subscribe(expired_subKey, function () {
    sub.on('message', function (event, key) {
      if(key.startsWith('ord')){// 订单过期， 修改状态
        Order.setExpire(key);
      }
    })
  })
...
async function createTicketOrder(uid:string, ticketId:string){
  let err, lock, flightData, ticketInfo:TicketInfo, orderInfo: OrderInfo, code = codes.ERR, data;
  console.log('createTicketOrder begin:', uid, ticketId, Date.now(), LOCK(ticketId), createMax+2000);
  [err, lock] = await to(getRedLock()!.lock(LOCK(ticketId), createMax+2000));
  do{
    if(err){
      code = codes.LOCK_ERR;
    ...
    if(!orderInfo){
      ...
      if(Math.random() <= createTicketOrderConfig.fail){// 有一定概率失败
      ...
      // 设置一个标记， 过期redis会通知
      [err,] = await to(Order.setExpireFlag(orderId, createTicketOrderConfig.expired, uid));
      if(err){
...
async function payTicketOrder(uid:string, orderId:string){
  let err, code = codes.ERR, data, lock, orderInfo:OrderInfo;
  ...
  do{
    ...
    // 查询订单状态
    if( !(orderInfo.curState === OrderState.CreateOrder || orderInfo.curState === OrderState.PayOrderFail) ){
    ...
    if(Math.random() <= payTicketOrderConfig.fail){// 有一定概率失败, 可能钱不足或者不想支付
      code = codes.THIRD_ERR;
...
async function cancelTicketOrder(uid:string, orderId:string){
  let err, lock, code = codes.ERR, orderInfo:OrderInfo;
  do{
    ...
    if(new Date() < orderInfo.expired  &&  
      (orderInfo.curState === OrderState.CreateOrder || orderInfo.curState === OrderState.PayOrderFail)){
...
async function getflightInfo(flightId:string){
  let err, data, flightInfo, code = codes.ERR;
  do{
